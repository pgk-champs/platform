...
test('показывает все главы трека, а не выборку', () => {
  ...
  expect(container.querySelectorAll('.tc-card')).toHaveLength(count('mobile'));
...
test('главы разложены по ярусам и не задваиваются', () => {
  ...
  const groups = container.querySelectorAll('.tc-group');
  expect(groups.length).toBeGreaterThan(1);
  const inGroups = [...groups].reduce((s, g) => s + g.querySelectorAll('.tc-card').length, 0);
...
test('карточка говорит, что внутри главы — вместо описания, которого нет', () => {
  ...
  const parts = container.querySelector('.tc-parts')?.textContent ?? '';
...
test('полоса прогресса появляется только у начатых глав', () => {
  ...
  expect(before.querySelectorAll('.tc-bar')).toHaveLength(0);

  store.setSectionRead('typing', 'intro');
  const { container: after } = render(<TrackChapters track="foundation" />);
  expect(after.querySelectorAll('.tc-bar').length).toBeGreaterThan(0);
});

test('ссылка ведёт на главу без расширения', () => {
  const { container } = render(<TrackChapters track="mobile" />);
  const href = container.querySelector('.tc-card')?.getAttribute('href') ?? '';
